...
var PY_CASES = [
{
  ...
  notes:["`shape` 看规模，`dtypes` 看类型（**日期读成 object 是最常见的坑**），`describe()` 看分布与异常值。", "养成习惯：先 `head()` 看几行真实数据，再动手写分析逻辑。", "如果 `dtypes` 里出现 `object`，通常意味着这列需要转换类型或做清洗。"],
  pitfalls:["跳过体检直接分析，结果因为类型不对（如日期是字符串）导致筛选/排序全错。"]
},
{
  ...
  output:"【users】行数 620\n  无缺失值\n  完全重复行: 0\n\n【events】行数 5362\n  无缺失值\n  完全重复行: 0\n\n【orders】行数 139\n  无缺失值\n  完全重复行: 0\n\nuser_id 重复条数: 0",
  notes:["`isna().sum()` 给出每列缺失数；除以行数才是\"缺失率\"，缺失率才可比较。", "`duplicated()` 查完全重复行；**业务主键重复**（如 user_id 重复）往往更致命，要单独查。", "缺失处理有三条路：删除、填充（均值/中位数/前值）、或**保留并单独标记**——选哪条取决于业务含义。"],
  pitfalls:["无脑 `dropna()` 会把有效数据一起删掉（例如\"未付费\"本来就是空值，不是缺失）。", "只看缺失数不看缺失率：100 条缺失在 1 万行和 100 万行里的严重程度完全不同。"]
},
{
  ...
  notes:["`pd.to_datetime()` 是日期分析的开关。转完后就能用 `.dt` 访问器取年/月/周/星期。", "`+ pd.Timedelta(days=1)` 做日期加减，比手写字符串拼接安全得多。", "`.dt.to_period('W')` 取自然周，是做周报/周留存的标准做法。"],
  pitfalls:["在字符串上做日期加减（如 `\"2026-08-31\" + 1`）——必错，且不会报错只会算错。", "忘记处理时区或格式不一致（如混有 \"2026/8/1\"）导致 `to_datetime` 报错或产生 NaT。"]
},
{
  ...
  notes:["多条件必须用 `&`（与）/`|`（或），**且每个条件都要加括号**——运算符优先级会让 `a > 1 & b < 2` 出错。", "`np.where(条件, 真值, 假值)` 是向量化的 if-else，比 `apply` 快得多。", "嵌套 `np.where` 可以处理多分支；分支超过 4 个时建议改用 `pd.cut`。"],
  pitfalls:["用 `and`/`or` 连接条件——pandas 里会直接报错（`ValueError: truth value ambiguous`）。", "写 `for` 循环逐行处理——慢且容易出错，几乎总有向量化替代方案。"]
},
{
  ...
  output:"按年龄段分层（cut）：\nage_band\n18-21    151\n22-25    170\n26-29    138\n30+      161\nName: count, dtype: int64\n\n按消费金额等频分 4 层（qcut）：\ngmv\n普通     31\n潜力     30\n优质     30\n高价值    31\nName: count, dtype: int64\n\n     人数   金额下限\ngmv           \n普通   31  54.43\n潜力   30  97.53\n优质   30 138.56\n高价值  31 176.72",
  notes:["`cut` 按**你指定的边界**切（可能各段人数悬殊）；`qcut` 按**分位数**切（各段人数接近）。", "用户分层通常用 `qcut`——因为运营希望每层都有人可运营。", "`qcut` 遇到大量重复值会报 \"Bin edges must be unique\"，此时改用 `cut` 或先 `rank()`。"],
  pitfalls:["用 `cut` 做分层后发现某层只有 3 个人——无法运营，该用 `qcut`。", "分箱边界随手写（如 0/50/100），与业务无关；边界应来自业务定义或分位数。"]
},
{
  ...
  notes:["`agg({新列名: 函数})` 可以一次产出多列，比分别算再 merge 清晰得多。", "`count` 数非空条数、`size` 数所有行（含空值），两者在有空值时结果不同——聚合前想清楚要哪个。", "算完立刻 `sort_values` 排序，输出才可读。"],
  pitfalls:["把 `count` 和 `size` 混用导致人数统计偏差（尤其是外连接后出现 NaN 时）。", "分组键里有空值，pandas 默认会把它丢掉（`dropna=True`）——需要时记得改成 `dropna=False`。"]
},
{
  ...
  notes:["**left join 的行数应等于左表行数**（除非右表有重复键）。行数变多说明右表键重复，数据会重复计数。", "聚合用户数要用 `nunique`（去重），用 `count` 会因为订单行数把用户数放大。", "多表关联后要检查列名，避免同名列变成 `col_x`/`col_y`。"],
  pitfalls:["merge 后行数暴涨却没发现，导致所有\"人数\"指标被放大数倍。", "用 `count` 统计用户数（应为 `nunique`）——这是最高频的指标计算错误之一。"]
},
{
  ...
  notes:["`index` 是行、`columns` 是列、`values` 是要算的字段、`aggfunc` 是算法（默认 mean，数人数要显式写 `nunique` 或 `count`）。", "`fill_value=0` 把空白填 0，输出更整洁。", "占比要用 `div(..., axis=0)` 按行归一化——写成 `axis=1` 会把百分比算错。", "**`observed=False`**：`pd.cut/qcut` 产生的是 categorical 类型，不写这个参数 pandas 会发弃用警告（未来版本行为会变）。虽然不影响结果，但生产代码里应显式声明。"],
  pitfalls:["忘了写 `aggfunc`，默认求出的是平均值而不是人数。", "归一化方向搞错（`axis` 写反），得到的百分比加总不等于 100%。"]
},
{
  ...
  notes:["`transform` 返回与原表等长的结果，可直接赋值成新列——这比 `groupby().agg()` 再 merge 回原表简洁得多。", "典型的\"组内占比\"公式：`组内计数 / 组内总数`，两个数都能用 transform 得到。", "`drop_duplicates()` 用来把\"广播后重复的行\"压回每个组合一行。"],
  pitfalls:["用 `agg` 得到的结果长度变了，直接赋值给原表会报错或错位——这时就该用 `transform`。", "把 `transform` 和 `apply` 混用，`apply` 返回结构不一致时容易得到嵌套结果。"]
},
{
  ...
  notes:["`dt.to_period('M')` 得到\"年月\"分组键（`dt.month` 只给月份数字，跨年会把 1 月混在一起）。", "`pct_change()` 直接算环比（小数），乘 100 才是百分比。", "`cumsum()` 算累计值，做增长曲线很常用。"],
  pitfalls:["用 `dt.month` 分组导致跨年数据混在一起（2025-12 和 2026-12 被算成同一个月）。", "第一个月的环比是 NaN（没有上月），展示时要处理，否则报错或显示怪异。"]
},
{
  ...
  notes:["**分母必须排除\"观察期不足\"的用户**（最后一天注册的人看不到次日行为），否则留存率被系统性低估。", "用 left join 判断留存：匹配上就是留存，`NaN` 就是流失——比 `isin` 更省内存且能保留所有人。", "`.assign(**{...})` 可以在链式写法里新增计算列，`**` 是为了支持中文列名。"],
  pitfalls:["分母没排除最后一天注册的用户——这是留存分析最常见、也最容易被忽略的错误。", "把\"次日留存\"算成\"次日内任意一天留存\"（口径不同，结论会差很多）。"]
},
{
  ...
  notes:["漏斗的核心是\"每步用集合去重求人数\"，再算相邻两步的转化率。", "同时给出\"相对上一步\"和\"相对首步\"两个口径——前者定位瓶颈，后者看整体效率。", "用 `set` 做去重比反复 `nunique` 更直观（数据量大时可用 `nunique`）。"],
  pitfalls:["漏斗步骤之间不是\"子集关系\"却当成漏斗（例如\"发帖用户\"不一定\"登录过\"），会算出超过 100% 的转化率。", "只看最终转化率不看中间环节，无法定位问题在哪一步。"]
},
{
  ...
  notes:["`sort_values` 后接 `groupby().head(n)` 是\"分组取 TopN\"最易读的写法。", "`rank(method='dense')` 处理并列名次（1,2,2,3），`method='min'` 则给 1,2,2,4。", "组内占比同样用 `transform('sum')` 广播回每行，避免 merge。"],
  pitfalls:["`groupby().head(n)` 前忘了先排序——取到的是\"前 n 行\"而不是\"最大的 n 个\"。", "并列名次处理不当（如用默认 `method='average'`）导致排名出现小数。"]
},
{
  ...
  notes:["`pearsonr` 返回 `(相关系数, p值)`：相关系数看强度（-1~1），p 值看是否显著（<0.05 通常认为显著）。", "**相关 ≠ 因果**：即使强相关，也不能说\"活跃导致付费\"——也可能是付费带来了更多活跃。", "相关系数接近 0 说明没有**线性**关系，但不代表完全无关（可能存在非线性关系，可看分组均值）。"],
  pitfalls:["只看相关系数不看 p 值 / 样本量：几十个样本的 r=0.3 很可能只是噪声。", "把相关性当成因果，据此下\"提升活跃就能提升付费\"的结论。"]
},
{
  ...
  notes:["卡方检验回答的是\"两组差异是否可能由随机造成\"，`p < 0.05` 才能认为差异显著。", "**统计显著 ≠ 业务显著**：样本极大时，0.02% 的提升也会显著，但不值得为此增加系统复杂度。", "卡方检验要求期望频数 ≥5（至少 80% 的格子）。这里期望频数都很大，条件满足。"],
  pitfalls:["p 值不显著却因为\"方向是正的\"就上线——等于在赌随机波动。", "为了凑显著性而反复观察/延长实验（peeking），会让假阳性率大幅上升。"]
},
{
...
var PY_CASE_META = {"updated": "2026-09-12", "intro": "Python 数据分析常用案例 · 每个案例都可直接复制到本地运行。代码下方的「运行结果」是用真实 pandas 跑出来的标准输出，不是手写的示意。", "env": "pandas 2.2 / numpy 2.2 / scipy 1.15 · 数据集为平台内置的 users / events / orders / channels 四张表", "groups": ["数据读取与体检", "数据清洗与加工", "聚合与合并", "时间序列与留存", "排名与统计"]};
